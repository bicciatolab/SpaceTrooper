...
# Visualizing cell centroids
To focus on cell centroids, use the `plotCentroids` function. It provides
...
plotCentroids(spe, size=0.5)
plotCentroids(spe, colourBy="Mean.DAPI", size=0.5)
```
...
the Area of each cell (`log2SignalDensity`) and the proportion between the 
...
```{r plot-zoom-fov}
plotZoomFovsMap(spe, fovs=201, colourBy="QC_score", csize=0.5, scaleBarMap=TRUE, scaleBarPol=FALSE)
plotZoomFovsMap(spe, fovs=201, colourBy="low_qcscore", csize=0.5, scaleBarMap=TRUE, scaleBarPol=FALSE)
```
