...
(xen <- readXeniumSPE(xeniumFolder, compute_missing_metrics=TRUE,
                        keep_polygons=TRUE))
colData(xen)
...
(mer <- readMerfishSPE(merscopeFolder, boundaries_type="parquet",
...
We can see in yellow and darkviolet that there are few cells with extreme values 
of log2AspectRatio and Area in micron.

```{r plot-polygons-fov-1}
plotPolygons(spe, colour_by="quality_score")
plotPolygons(spe, colour_by="is_qscore_flags")
```
